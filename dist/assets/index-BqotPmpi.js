(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const t of document.querySelectorAll('link[rel="modulepreload"]'))l(t);new MutationObserver(t=>{for(const c of t)if(c.type==="childList")for(const r of c.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&l(r)}).observe(document,{childList:!0,subtree:!0});function s(t){const c={};return t.integrity&&(c.integrity=t.integrity),t.referrerPolicy&&(c.referrerPolicy=t.referrerPolicy),t.crossOrigin==="use-credentials"?c.credentials="include":t.crossOrigin==="anonymous"?c.credentials="omit":c.credentials="same-origin",c}function l(t){if(t.ep)return;t.ep=!0;const c=s(t);fetch(t.href,c)}})();const f=String.raw`<svg
  class="watch-art"
...
</svg>`,o=document.documentElement,g=document.querySelector("[data-assembly-hero]"),m=document.querySelector("[data-watch-mount]"),y=Array.from(document.querySelectorAll("[data-chapter]")),i=Array.from(document.querySelectorAll("[data-part-item]")),M=document.querySelector("[data-progress-value]"),d=window.matchMedia("(prefers-reduced-motion: reduce)");o.classList.add("js");m&&(m.innerHTML=f);const v=Array.from(document.querySelectorAll("[data-explode-x]")).map(a=>({element:a,x:Number(a.dataset.explodeX??0),y:Number(a.dataset.explodeY??0),rotation:Number(a.dataset.explodeRotate??0),start:Number(a.dataset.assembleStart??0),end:Number(a.dataset.assembleEnd??.82)}));let p=0,u=-1;const w=(a,e=0,s=1)=>Math.min(s,Math.max(e,a)),x=a=>{const e=w(a);return e*e*(3-2*e)},A=a=>{if(a!==u){u=a;for(const[e,s]of y.entries()){const l=d.matches||e===a;s.classList.toggle("is-active",l),s.setAttribute("aria-hidden",l?"false":"true")}}},L=()=>{if(p=0,!g)return;const a=g.getBoundingClientRect(),e=Math.max(1,a.height-window.innerHeight),s=d.matches?1:w(-a.top/e),l=x(s);o.style.setProperty("--hero-progress",l.toFixed(4)),o.style.setProperty("--intro-opacity",String(d.matches?1:w(1-s*4))),o.style.setProperty("--hero-light-x",`${12+l*48}%`),o.style.setProperty("--hero-glow-x",`${45+l*10}%`),o.style.setProperty("--hero-fill",`${l*100}%`);for(const r of v){const h=x((l-r.start)/Math.max(.001,r.end-r.start)),_=1-h;r.element.style.transform=`translate(${(r.x*_).toFixed(2)}px, ${(r.y*_).toFixed(2)}px) rotate(${(r.rotation*_).toFixed(2)}deg)`,r.element.style.opacity=String(.42+h*.58)}const t=d.matches?y.length-1:Math.min(4,Math.floor(s*5));A(t);const c=d.matches?i.length-1:Math.min(i.length-1,Math.floor(s*i.length));for(const[r,h]of i.entries())h.classList.toggle("is-active",r<=c),r===c?h.setAttribute("aria-current","step"):h.removeAttribute("aria-current");M&&(M.value=`0${t}—04`)},n=()=>{p||(p=window.requestAnimationFrame(L))};window.addEventListener("scroll",n,{passive:!0});window.addEventListener("resize",n,{passive:!0});d.addEventListener("change",n);n();const b=new IntersectionObserver(a=>{for(const e of a)e.isIntersecting&&(e.target.classList.add("is-visible"),b.unobserve(e.target))},{rootMargin:"0px 0px -12%",threshold:.12});for(const a of document.querySelectorAll("[data-reveal]"))d.matches?a.classList.add("is-visible"):b.observe(a);
